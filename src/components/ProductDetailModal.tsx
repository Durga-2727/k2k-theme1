        ...
        >
          ...
          <div className="flex justify-between items-center px-5 pt-5 pb-3 border-b border-slate-100 bg-white z-20">
            <div className="pr-8">
              ...
              <span className="text-[10px] text-[#0F766E] font-bold block mt-0.5 tracking-widest uppercase font-mono">{product.localName}</span>
            </div>
            <div className="flex items-center gap-1.5 shrink-0">
              <button
                id="detail-wishlist-toggle"
                onClick={() => onToggleWishlist(product.id)}
                className="p-2 bg-[#F5E6CA] hover:bg-slate-100 border border-slate-200 text-gray-700 hover:text-rose-500 rounded-xl transition-all cursor-pointer shadow-xs"
              ...
              <button
                id="detail-close-btn"
                onClick={onClose}
                className="p-2 bg-[#F5E6CA] hover:bg-slate-100 border border-slate-200 text-[#1F2937] hover:text-red-650 rounded-xl transition-all cursor-pointer shadow-xs"
                title="Close details"
              >
                <X className="h-4 w-4" />
              </button>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            <div className="space-y-3">
              <div
                className="relative aspect-video rounded-2xl overflow-hidden w-full bg-[#F5E6CA] border border-slate-100 shadow-inner group/zoom select-none cursor-zoom-in"
              ...
              >
                ...
                <div className="absolute bottom-3 left-3 flex gap-1.5">
                  <span className="bg-[#99F6E4]/90 backdrop-blur-md text-white text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded-lg shadow-xs">
                  ...
                  {product.images.map((imgUrl, idx) => (
                    <button
                      ...
                      className={`w-14 h-11 rounded-lg overflow-hidden border-2 transition-all shrink-0 cursor-pointer ${
                        activeImageIndex === idx ? 'border-[#0F766E] scale-102 shadow-xs' : 'border-slate-200 opacity-70 hover:opacity-100'
                      }`}
            ...
            <div className="bg-[#F5E6CA] border border-slate-200/50 p-3 rounded-xl grid grid-cols-2 gap-3 text-[10px] font-mono">
            ...
            <div className="p-3 bg-[#99F6E4]/40 border border-[#99F6E4]/50 rounded-xl flex items-center justify-between">
              <div>
                ...
                <div className="flex items-baseline gap-1 mt-0.5">
                  <span className="text-xs font-bold text-[#0F766E]">₹</span>
                  <span className="text-xl font-bold text-[#0F766E]">{currentPrice}</span>
                  <span className="text-[10px] text-slate-400 font-sans">/{selectedWeight} Box</span>
                </div>
              </div>
              <span className="px-2 py-0.5 text-[8px] bg-[#99F6E4]/10 text-[#0F766E] font-bold uppercase rounded-lg font-mono">
                Price Locked
              </span>
            </div>
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 text-[10px] font-bold rounded-lg border transition-all cursor-pointer ${
                        isSelect
                          ...
                          : 'bg-[#F5E6CA] border-slate-200 text-slate-600 hover:bg-slate-100'
                      }`}
                    >
                      {w}
                    </button>
                    ...
                    return (
                      <button
                        ...
                        className={`px-2.5 py-1.5 text-[10px] font-bold rounded-lg border transition-all cursor-pointer ${
                          isSelect
                            ? 'bg-[#0F766E] border-[#0F766E] text-white shadow-xs'
                            : 'bg-[#F5E6CA] border-slate-200/50 text-slate-600 hover:bg-slate-100'
                        }`}
                      >
                        {o}
                      </button>
            ...
            <div className="bg-[#F5E6CA]/50 border border-slate-200/30 rounded-2xl p-4">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`pb-1.5 text-[10px] uppercase font-bold border-b-2 font-mono transition-colors cursor-pointer ${
                      activeTab === t.id ? 'border-[#0F766E] text-[#0F766E]' : 'border-transparent text-slate-400 hover:text-slate-700'
                    }`}
                  >
                    {t.label}
                  </button>
                ...
                {activeTab === 'desc' && (
                  <div className="space-y-3 font-sans">
                    ...
                    <div className="p-2.5 bg-white rounded-lg border border-slate-100 flex items-center gap-2 shadow-inner">
                      <div className="w-8 h-8 bg-[#99F6E4] rounded-full flex items-center justify-center font-bold text-[#0F766E] text-[10px] overflow-hidden shrink-0 border border-[#99F6E4]">
                    ...
                    {reviewsList.map((rev, idx) => (
                      <div key={idx} className="bg-white border border-slate-100 p-2.5 rounded-lg flex justify-between items-start gap-2 shadow-xs">
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5">
                            <span className="w-4 h-4 rounded-full bg-[#99F6E4] text-[#0F766E] flex items-center justify-center font-bold text-[8px]">
                              {rev.name.charAt(0)}
                            </span>
                ...
                <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-none">
                  {relatedProducts.map((p) => (
                    <button
                      ...
                      className="w-36 bg-white border border-slate-100 hover:border-[#99F6E4] p-2 rounded-xl shadow-xs hover:shadow-sm transition-all duration-300 text-left shrink-0 cursor-pointer flex flex-col justify-between aspect-[3/4]"
                      onClick={() => onSelectProduct(p)}
                    >
                      <div className="aspect-video w-full rounded-lg overflow-hidden bg-[#F5E6CA]">
                        <ProductImage
                          productId={p.id}
                          src={p.images[0]}
                          alt={p.name}
                      ...
                      <div className="mt-1">
                        <span className="text-[7px] font-bold text-[#0F766E] block uppercase font-mono leading-none">{p.localName}</span>
                      ...
                      <div className="flex justify-between items-center pt-1 border-t border-slate-50 mt-1">
                        <strong className="text-[10px] font-extrabold text-[#0F766E]">₹{p.price}</strong>
                        <span className="text-[8px] text-[#0F766E] font-mono">View →</span>
                      </div>
                    </button>
                  ))}
                </div>
